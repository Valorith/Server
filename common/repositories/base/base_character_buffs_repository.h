...
public:
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			e.caster_is_client = row[5] ? static_cast<uint8_t>(strtoul(row[5], nullptr, 10)) : 0;
			...
			e.suppressed       = row[18] ? static_cast<uint8_t>(strtoul(row[18], nullptr, 10)) : 0;
	...
	)
	{
		...
		v.push_back(columns[5] + " = " + std::to_string(e.caster_is_client));
		...
		v.push_back(columns[18] + " = " + std::to_string(e.suppressed));
	...
	)
	{
		...
		v.push_back(std::to_string(e.caster_is_client));
