...
RULE_INT(Character, MaxDiscoverItemStatus, 0, "Characters with an account status above this value do not add entries to discovered items")
...
RULE_BOOL(Spells, BuffTicAttributionFallback, true, "Keep DoTs ticking and attribute buff tic messages using the buff's stored caster name when the caster mob is dead or gone from zone")
RULE_CATEGORY_END()

RULE_CATEGORY(Combat)
RULE_BOOL(Combat, GroupRaidCombatLogParity, true, "In-zone group/raid members of any party to a combat event receive its combat packets/messages regardless of proximity")
RULE_BOOL(Combat, HealBystanderMessages, true, "Send third-person heal/HoT messages to bystanders and group/raid members")
...
RULE_BOOL(EvolvingItems, ShowExperienceMessages, false, "Announce evolving item experience gains in the experience chat channel. Formatting follows Character:ShowExpValues.")
