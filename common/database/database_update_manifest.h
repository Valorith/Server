...
CREATE TABLE IF NOT EXISTS `expedition_template_request_npcs` (
	...
	KEY `idx_expedition_request_lookup` (`zone_id`, `zone_version`, `npc_type_id`, `spawn2_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `expedition_template_event_npcs` (
	...
	`complete_on_spawn` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0',
	`loot_protected` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0',
	PRIMARY KEY (`id`),
...
ALTER TABLE `trader_audit`
	...
	ManifestEntry{
		.version = 9352,
		.description = "2026_08_31_selectable_rewards_content.sql",
		.check = "SELECT 1",
		.condition = "not_empty",
		.match = "",
		.sql = R"SQL(
...
DO EXISTS (
	...
	ManifestEntry{
		.version = 9353,
		.description = "2026_08_31_selectable_rewards_state.sql",
		.check = "SELECT 1",
		.condition = "not_empty",
		.match = "",
		.sql = R"SQL(
