...
void TaskManager::TaskSetSelector(Client* client, Mob* mob, int task_set_id, bool ignore_cooldown)
{
	...
	while (iterator != m_task_sets[task_set_id].end() && task_list.size() < MAXCHOOSERENTRIES) {
		...
		if (SupportsTaskRewardMethod(client, task_data) &&
			(all_enabled || client_task_state->IsTaskEnabled(task)) &&
			CanClientSeeTaskOffer(task, player_level, player_class) &&
...
void TaskManager::TaskQuestSetSelector(Client* client, Mob* mob, const std::vector<int>& tasks, bool ignore_cooldown)
{
	...
	for (int i = 0; i < tasks.size() && task_list.size() < MAXCHOOSERENTRIES; ++i) {
		...
		if (SupportsTaskRewardMethod(client, task_data) &&
			CanClientSeeTaskOffer(task, player_level, player_class) &&
			!client_task_state->IsTaskActive(task) &&
...
void TaskManager::SendSharedTaskSelector(Client* client, Mob* mob, const std::vector<int>& task_list, const SharedTaskRequest& request)
{
	...
	valid_task_list.reserve(supported_tasks.size());
