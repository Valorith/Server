...
namespace {
	...
	std::string PopupLines(const std::vector<std::string>& lines)
	{
		std::string body;
	...
	void SendActionLink(Client* c, const std::string& command, const std::string& label, const std::string& description = {})
	{
		if (!c) {
			return;
		}

		std::string line = fmt::format("  {}", Saylink::Silent(command, fmt::format("[ {} ]", label)));
	...
	void SendPrefillActionLink(Client* c, const std::string& command, const std::string& label, const std::string& description = {})
	{
		if (!c) {
			return;
		}

		std::string line = fmt::format("  {}", Saylink::Create(command, false, fmt::format("[ {} ]", label)));
		if (!description.empty()) {
			line += fmt::format(" - {}", description);
		}
		c->Message(Chat::White, line.c_str());
	...
	void SendCommandConfirmationPopup(
		Client* c,
		const std::string& title,
		const std::vector<std::string>& body_lines,
		const std::string& confirm_command,
		const std::string& confirm_button = "Confirm"
	)
	{
		if (!c) {
			return;
		...
		c->SetEntityVariable(DIAWIND_RESPONSE_ONE_KEY, confirm_command);
		c->SetEntityVariable(DIAWIND_RESPONSE_TWO_KEY, "");
		const std::string body = PopupLines(body_lines);
		c->SendFullPopup(title.c_str(), body.c_str(), POPUPID_DIAWIND_ONE, POPUPID_DIAWIND_TWO, 1, 0, confirm_button.c_str(), "Cancel");
		c->Message(Chat::Yellow, fmt::format("{} confirmation popup sent.", title).c_str());
	}
	...
	void ShowCreateHelp(Client* c)
	{
		...
		c->Message(Chat::White, "  It uses your current zone/version, your spot as the zone-in and compass, the zone safe point as the return, 6-54 player limits, a 6h duration, and a 1d replay timer.");
	...
	std::string BossCompletionSummary(const ExpeditionDB::Template& template_data)
	{
		...
		if (names.empty()) {
			return "Target the boss, then use #expedition boss or #expedition boss loot.";
		}

		if (names.size() == 1) {
			return fmt::format("{} boss lockout: {}. Target another boss and use #expedition boss loot to add it.", names.size(), names.front());
		}

		if (names.size() == 2) {
			return fmt::format("{} boss lockouts: {} and {}. Target another boss and use #expedition boss loot to add it.", names.size(), names[0], names[1]);
		}

		return fmt::format("{} boss lockouts: {}, {}, and {} more. Target another boss and use #expedition boss loot to add it.", names.size(), names[0], names[1], names.size() - 2);
	}
	...
	WizardGuidance BuildWizardGuidance(Client* c, const ExpeditionDB::Template& template_data, const ExpeditionDB::ValidationResult& validation)
	{
		...
		if (template_data.events.empty()) {
			return {
				...
				npc ? "#expedition boss loot" : "#expedition boss",
				npc ? "Create Boss Event From Target" : "Show Boss Step",
				{
					{"#expedition boss", "Use Target Boss"},
					{"#expedition boss loot", "Use Target Boss With Loot"},
		...
		if (event_data && event_data->lock_on_success && event_data->lockout_seconds == 0) {
			return {
				...
				"#expedition event lockout 156h",
				"Set 6.5 Day Event Lockout",
				{
					{"#expedition event lockout 156h", "6.5 Day Lockout"},
					{"#expedition boss", "Use Target Boss"},
					{"#expedition event replay 1d", "1 Day Replay"}
				}
			};
		}
		...
		if (template_data.replay_lockout_seconds == 0) {
			return {
				...
				"#expedition set replay 1d",
				"Set 1 Day Replay",
				{
					{"#expedition set replay 30m", "30 Minute Replay"},
					{"#expedition set replay 1d", "1 Day Replay"},
		...
		if (event_data && npc && FindEventNpc(*event_data, *npc) && !HasLootProtectedNpc(*event_data)) {
			return {
				...
				"Protect Target Loot",
				{
					{"#expedition boss loot", "Protect Target Loot"},
					{"#expedition boss noloot", "Leave Loot Unprotected"},
		...
		return {
			...
			{
				{"#expedition test request", "Simulate Request"},
				{"#expedition test create", "Create Live Test..."},
				{"#expedition test move", "Move Into Expedition..."},
				{"#expedition show", "Show"}
			}
		};
	}
	...
	void ShowRequestNpcHelp(Client* c)
	{
		...
		SendHelpLink(c, "#expedition request remove", "review targeted request NPC removal");
	...
	void ShowEventHelp(Client* c)
	{
		...
		SendHelpLink(c, "#expedition event npc remove", "review targeted NPC removal from selected event");
		...
		SendActionGroup(c, "Event Actions", {
			...
			{"#expedition event lockout 156h", "6.5d Lockout"},
			{"#expedition", "Home / Overview"}
		});
		if (const auto* template_data = SelectedTemplate(c)) {
	...
	void ShowGroupedBossEventCard(Client* c, const ExpeditionDB::Event* event_data)
	{
		...
		if (!event_data) {
			...
			SendActionRow(c, {
				{"#expedition event list", "Back to Events"},
				{"#expedition show", "Back to Expedition"}
			});
			SendCardBottom(c);
			return;
		}
		...
		SendPrefillActionLink(c, fmt::format("#expedition event rename {} \"{}\"", event_data->id, event_data->event_name), "Rename", "edit the event name");
		SendActionRow(c, {
	...
	void ShowTestHelp(Client* c)
	{
		...
		c->Message(Chat::White, "Simulation commands are safe. Commands that create/move/apply live state require confirmation.");
		SendHelpLink(c, "#expedition test request", "simulate targeted request NPC phrase flow");
		SendHelpLink(c, "#expedition test create", "review live test expedition creation");
		SendHelpLink(c, "#expedition test move", "review move into your current expedition");
		SendHelpLink(c, "#expedition test lockout", "review selected-event lockout application");
		SendHelpLink(c, "#expedition test loot", "review DB loot-event protection re-application");
	...
	void ShowPresetHelp(Client* c)
	{
		...
		SendHelpLink(c, "#expedition preset raid", "6-54 players, 6 hour duration, 1 day replay");
	...
	void ShowActionHelp(Client* c)
	{
		...
		SendActionGroup(c, "Runtime Actions", {
			...
			{"#expedition action add replay 1d", "1d Replay"},
			{"#expedition action add lockout 156h", "6.5d Lockout"},
	...
	void ShowAdvancedHelp(Client* c)
	{
		...
		SendHelpLink(c, "#expedition delete <id|name>", "review unpublished template deletion");
	...
	// Render one numbered roster NPC line, plus clickable actions when it is currently spawned in-zone.
	void RenderRosterNpc(
		Client* c,
		uint32_t number,
		const std::string& label,
	...
	{
		NPC* live = ResolveLiveRosterNpc(npc_type_id, spawn2_id);
		std::string name = ExpeditionDB::NpcTypeName(npc_type_id);
		if (name.empty()) {
			name = "unknown";
		}
		const std::string spawn_label = spawn2_id != 0 ? fmt::format(" spawn {}", spawn2_id) : "";
		c->Message(live ? Chat::Green : Chat::Gray, fmt::format(
			"   {:>2}. {}: {} ({}){}{}  -  {}",
			number, label, name, npc_type_id, spawn_label, extra, live ? "SPAWNED" : "not spawned"
		).c_str());
		if (live || show_manage) {
			const std::string spawn_arg = spawn2_id != 0 ? fmt::format(" {}", spawn2_id) : "";
			std::string actions = "       ";
			if (live) {
				actions += fmt::format(
					"{}    {}",
					Saylink::Silent(fmt::format("#expedition goto {}{}", npc_type_id, spawn_arg), "[ GoTo ]"),
					Saylink::Silent(fmt::format("#expedition target {}{}", npc_type_id, spawn_arg), "[ Target ]")
				);
			}
			if (show_manage) {
				if (live) {
				...
				actions += Saylink::Silent(
					fmt::format("#expedition manage {} {} {} {}", template_id, event_id, npc_type_id, spawn2_id),
					"[ Manage ]"
				);
			}
			c->Message(Chat::White, actions.c_str());
		}
	}

	const ExpeditionDB::Event* FindTemplateEventByID(const ExpeditionDB::Template& template_data, uint32_t event_id)
	{
		auto it = std::ranges::find_if(template_data.events, [event_id](const ExpeditionDB::Event& event_data) {
	...
	const ExpeditionDB::EventNpc* FindEventNpcByMapping(const ExpeditionDB::Event& event_data, uint32_t npc_type_id, uint32_t spawn2_id)
	{
		auto it = std::ranges::find_if(event_data.npcs, [&](const ExpeditionDB::EventNpc& event_npc) {
			return event_npc.npc_type_id == npc_type_id && event_npc.spawn2_id == spawn2_id;
		});

		return it != event_data.npcs.end() ? &*it : nullptr;
	}

	void ShowMappedNpcManageMenu(
		Client* c,
		const ExpeditionDB::Template& template_data,
		const ExpeditionDB::Event& event_data,
		const ExpeditionDB::EventNpc& event_npc
	)
	{
		...
		const std::string base_command = fmt::format(
			"#expedition manage action {} {} {} {}",
			template_data.id,
			event_data.id,
		...
		SendCardTop(c, fmt::format("{}: {} ({})", role_label, npc_name, event_npc.npc_type_id));
		if (!live) {
			c->Message(Chat::Gray, "  This NPC is not spawned right now. DB mapping actions are still available.");
		}
		SendInfoLine(c, "Event", event_data.event_name);
		SendInfoLine(c, "Trigger", trigger);
		SendInfoLine(c, "Lockout", Duration(event_data.lockout_seconds));
		SendInfoLine(c, "Loot protection", OnOff(loot_protected));
		c->Message(Chat::White, ChatSeparator());

		if (live) {
			const std::string spawn_arg = event_npc.spawn2_id != 0 ? fmt::format(" {}", event_npc.spawn2_id) : "";
			SendActionRow(c, {
				{fmt::format("#expedition goto {}{}", event_npc.npc_type_id, spawn_arg), "GoTo"},
				{fmt::format("#expedition target {}{}", event_npc.npc_type_id, spawn_arg), "Target"}
			});
		}

		SendActionRow(c, {
			{fmt::format("{} loot {}", base_command, loot_protected ? "off" : "on"), loot_protected ? "Loot Protection: Off" : "Loot Protection: On"},
			{"#expedition event lockout", "Set Lockout..."},
			{fmt::format("{} remove", base_command), ExpeditionDB::EventNpcRemovalDeletesEvent(event_data, event_npc) ? "Remove Boss/Event..." : "Remove from Expedition..."}
		});
		SendActionRow(c, {
			{fmt::format("{} death {}", base_command, event_npc.complete_on_death ? "off" : "on"), event_npc.complete_on_death ? "Death Trigger Off" : "Death Trigger On"},
			{fmt::format("{} spawn {}", base_command, event_npc.complete_on_spawn ? "off" : "on"), event_npc.complete_on_spawn ? "Spawn Trigger Off" : "Spawn Trigger On"}
		});
		c->Message(Chat::White, fmt::format("  Rename event:  {}",
			Saylink::Create(fmt::format("#expedition event rename {} \"{}\"", event_data.id, event_data.event_name), false, "[ Rename Event ]")).c_str());
		c->Message(Chat::White, "  Navigate:");
		std::vector<std::pair<std::string, std::string>> nav_actions;
		if (ExpeditionDB::IsGroupedBossCompletionMode(event_data)) {
			nav_actions.emplace_back(fmt::format("#expedition event group {}", event_data.id), "Back to Boss Group");
		}
		nav_actions.emplace_back(fmt::format("#expedition show {}", template_data.id), "Back to Expedition");
		nav_actions.emplace_back("#expedition event list", "Event List");
		SendActionRow(c, nav_actions);
		SendCardBottom(c);
	}
	...
	void ShowTemplate(Client* c, const ExpeditionDB::Template& template_data)
	{
		...
		for (const auto& event_data : template_data.events) {
			...
			if (ExpeditionDB::IsGroupedBossCompletionMode(event_data)) {
				SendActionRow(c, {
					{fmt::format("#expedition event group add {}", event_data.id), "Add Target to Group"},
					{fmt::format("#expedition event group {}", event_data.id), "Manage Group"},
					{fmt::format("#expedition event remove {}", event_data.id), "Delete Event"}
				});
			}
			else {
			...
			for (const auto& event_npc : event_data.npcs) {
				...
				RenderRosterNpc(c, ++roster_index, "Boss", event_npc.npc_type_id, event_npc.spawn2_id, template_data.id, event_data.id, true,
			...
			for (const auto& event_npc : event_data.npcs) {
				...
				RenderRosterNpc(c, ++roster_index, "Chest", event_npc.npc_type_id, event_npc.spawn2_id, template_data.id, event_data.id, true,
		...
		for (const auto& request_npc : template_data.request_npcs) {
			RenderRosterNpc(c, ++requester_index, "Requester", request_npc.npc_type_id, request_npc.spawn2_id, template_data.id, 0, false,
	...
	void ShowPreview(Client* c, const ExpeditionDB::Template& template_data)
	{
		...
		c->Message(Chat::White, ChatSeparator());
		SendActionRow(c, {
			{fmt::format("#expedition show {}", template_data.id), "Back to Expedition"},
			{"#expedition", "Back to List"}
		});
	}
	...
	void ShowFixes(Client* c, const ExpeditionDB::Template& template_data)
	{
		...
		SendActionLink(c, "#expedition fix zonein", "Use Current Zone-In", "use your current location as zone-in");
		SendActionLink(c, "#expedition fix safereturn", "Use Current Safe Return", "use your current location as safe return");
		SendActionLink(c, "#expedition fix requestnpc", "Add Target Request NPC", "add your targeted NPC as request NPC");
		SendActionLink(c, "#expedition fix replay 1d", "Set 1d Replay", "set a 1 day replay lockout");
		SendActionLink(c, "#expedition preset raid", "Apply Raid Defaults", "apply raid defaults");
		SendActionRow(c, {
			{fmt::format("#expedition show {}", template_data.id), "Back to Expedition"},
			{"#expedition", "Back to List"}
		});
...
namespace {
	...
	void HandleManageNpc(Client* c, const Seperator* sep);
...
void HandleConfig(Client* c, const Seperator* sep)
{
		...
		if (action == "duration") {
			...
			c->Message(Chat::Green, fmt::format("Set player limits to {} ({}-{} players).", size, min_players, max_players).c_str());
		}
...
void HandleCreate(Client* c, const Seperator* sep)
{
	...
	c->Message(Chat::Green, fmt::format(
		"Created expedition [{}] \"{}\" with raid defaults - now in edit mode. Use [ Rename ] below to change the name, or target NPCs to add bosses / requesters.",
		id, name
	).c_str());
...
void HandleRequest(Client* c, const Seperator* sep)
{
	...
	if (action == "removeid") {
		...
		if (strcasecmp(sep->arg[7], "confirm") != 0) {
			SendCommandConfirmationPopup(c, "Remove Request NPC", {
				fmt::format("Remove requester NPC type {} from [{}]?", type_id, template_data->name),
				fmt::format("Zone: {}", ZoneVersionLabel(zone_id, zone_version)),
				"This removes the DB request NPC mapping."
			}, fmt::format("#expedition request removeid {} {} {} {} confirm", type_id, spawn2_id, zone_id, zone_version), "Remove");
			return;
		}
	...
	if (action == "remove") {
		if (strcasecmp(sep->arg[3], "confirm") != 0) {
			SendCommandConfirmationPopup(c, "Remove Request NPC", {
				fmt::format("Remove targeted request NPC [{}] from expedition [{}]?", NpcLabel(*npc), template_data->name),
				"This removes the DB request NPC mapping."
			}, "#expedition request remove confirm", "Remove");
			return;
		}
...
void HandleBoss(Client* c, const Seperator* sep)
{
	if (strcasecmp(sep->arg[2], "help") == 0) {
		...
		c->Message(Chat::White, "Example: #expedition boss loot");
		c->Message(Chat::White, "Example: #expedition boss chest");
		return;
	}
	...
	NPC* npc = RequireTargetNpc(c, "Target the boss NPC, then run #expedition boss or #expedition boss loot.");
	if (!npc) {
		return;
	}
...
void HandleChest(Client* c, const Seperator* sep)
{
		...
		const auto* event_data = SelectedEvent(c, *template_data);
		if (!event_data) {
			c->Message(Chat::Red, "Add or select the boss event first. Use #expedition boss chest after targeting the boss.");
			return;
		}
...
void ShowChestLinkChooser(Client* c, const ExpeditionDB::Template& template_data, NPC& npc, uint32_t entity_id)
{
		...
		c->Message(Chat::White, ChatSeparator());
		SendActionRow(c, {{"#expedition edit on", "Back to Edit"}});
		SendCardBottom(c);
}
...
void HandleGoto(Client* c, const Seperator* sep)
{
		if (!IsStrictUnsigned(sep->arg[2])) {
			c->Message(Chat::Red, "Usage: #expedition goto <npc_type_id> [spawn2_id]");
			return;
		}
		const uint32_t npc_type_id = Strings::ToUnsignedInt(sep->arg[2]);
		const uint32_t spawn2_id = IsStrictUnsigned(sep->arg[3]) ? Strings::ToUnsignedInt(sep->arg[3]) : 0;
		NPC* npc = ResolveLiveRosterNpc(npc_type_id, spawn2_id);
		if (!npc) {
...
{
		if (!IsStrictUnsigned(sep->arg[2])) {
			c->Message(Chat::Red, "Usage: #expedition target <npc_type_id> [spawn2_id]");
			return;
		}
		const uint32_t npc_type_id = Strings::ToUnsignedInt(sep->arg[2]);
		const uint32_t spawn2_id = IsStrictUnsigned(sep->arg[3]) ? Strings::ToUnsignedInt(sep->arg[3]) : 0;
...
void HandleManageAction(Client* c, const Seperator* sep)
{
		if (
			!IsStrictUnsigned(sep->arg[3]) ||
			!IsStrictUnsigned(sep->arg[4]) ||
			!IsStrictUnsigned(sep->arg[5]) ||
			!IsStrictUnsigned(sep->arg[6])
		) {
			c->Message(Chat::Red, "Usage: #expedition manage action <template_id> <event_id> <npc_type_id> <spawn2_id> <loot|death|spawn|remove> [on|off|confirm]");
			return;
		}

		const uint32_t template_id = Strings::ToUnsignedInt(sep->arg[3]);
		const uint32_t event_id = Strings::ToUnsignedInt(sep->arg[4]);
		const uint32_t npc_type_id = Strings::ToUnsignedInt(sep->arg[5]);
		const uint32_t spawn2_id = Strings::ToUnsignedInt(sep->arg[6]);
		const std::string action = Strings::ToLower(sep->arg[7]);

		const auto* template_data = ExpeditionDB::FindTemplate(template_id);
		if (!template_data) {
			c->Message(Chat::Red, "Expedition template not found.");
			return;
		}

		const auto* event_data = FindTemplateEventByID(*template_data, event_id);
		if (!event_data) {
			c->Message(Chat::Red, "Event not found.");
			return;
		}

		const auto* event_npc = FindEventNpcByMapping(*event_data, npc_type_id, spawn2_id);
		if (!event_npc) {
			c->Message(Chat::Red, "NPC mapping not found for that event.");
			return;
		}
		const std::string event_name = event_data->event_name;
		...
		if (action == "remove") {
			if (strcasecmp(sep->arg[8], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Remove Event NPC", {
					fmt::format("Remove NPC mapping [{}] from event [{}]?", NpcMappingLabel(npc_type_id, spawn2_id), event_name),
					"This changes the DB event mapping."
				}, fmt::format("#expedition manage action {} {} {} {} remove confirm", template_id, event_id, npc_type_id, spawn2_id), "Remove");
				return;
			}

			const bool removes_event = ExpeditionDB::EventNpcRemovalDeletesEvent(*event_data, *event_npc);
			if (!ExpeditionDB::DeleteEventNpc(content_db, event_id, npc_type_id, spawn2_id)) {
				c->Message(Chat::Red, fmt::format("Failed to remove NPC mapping for event [{}].", event_name).c_str());
				return;
			}
			...
			if (removes_event) {
				...
				c->Message(Chat::Green, fmt::format("Saved: removed boss mapping and event [{}].", event_name).c_str());
			}
			else {
				c->Message(Chat::Green, fmt::format("Saved: removed NPC mapping from event [{}].", event_name).c_str());
			}
			RefreshBuilderView(c);
			return;
		}
		...
		bool enabled = true;
		if (sep->arg[8][0] != '\0' && !ParseOnOffArg(sep->arg[8], enabled)) {
			c->Message(Chat::Red, "Usage: #expedition manage action <template_id> <event_id> <npc_type_id> <spawn2_id> <loot|death|spawn> on|off");
			return;
		}
		...
		std::string label;
		if (action == "loot") {
			ok = ExpeditionDB::SetEventNpcLoot(content_db, event_id, npc_type_id, spawn2_id, enabled);
			label = "loot protection";
		}
		else if (action == "death") {
			ok = ExpeditionDB::SetEventNpcCompleteOnDeath(content_db, event_id, npc_type_id, spawn2_id, enabled);
			label = "complete-on-death";
		}
		else if (action == "spawn") {
			ok = ExpeditionDB::SetEventNpcCompleteOnSpawn(content_db, event_id, npc_type_id, spawn2_id, enabled);
			label = "complete-on-spawn";
		}
		else {
			c->Message(Chat::Red, "Usage: #expedition manage action <template_id> <event_id> <npc_type_id> <spawn2_id> <loot|death|spawn|remove> [on|off|confirm]");
			return;
		...
		if (!ok) {
			c->Message(Chat::Red, fmt::format("Failed to set {} for event [{}].", label, event_name).c_str());
			return;
		}

		c->Message(Chat::Green, fmt::format("Saved: event [{}] NPC [{}] {} is [{}].",
			event_name, ExpeditionDB::NpcTypeLabel(npc_type_id), label, OnOff(enabled)).c_str());
		if (const auto* refreshed_template = ExpeditionDB::FindTemplate(template_id)) {
			if (const auto* refreshed_event = FindTemplateEventByID(*refreshed_template, event_id)) {
				if (const auto* refreshed_npc = FindEventNpcByMapping(*refreshed_event, npc_type_id, spawn2_id)) {
					ShowMappedNpcManageMenu(c, *refreshed_template, *refreshed_event, *refreshed_npc);
					return;
				}
			}
		}
...
// Open the GM boss/chest management menu from a configured mapping, even if it is not spawned.
void HandleManageNpc(Client* c, const Seperator* sep)
{
		if (Strings::EqualFold(sep->arg[2], "action")) {
			HandleManageAction(c, sep);
			return;
		}

		if (
			!IsStrictUnsigned(sep->arg[2]) ||
			!IsStrictUnsigned(sep->arg[3]) ||
			!IsStrictUnsigned(sep->arg[4]) ||
			!IsStrictUnsigned(sep->arg[5])
		) {
			c->Message(Chat::Red, "Usage: #expedition manage <template_id> <event_id> <npc_type_id> <spawn2_id>");
			return;
		}

		const uint32_t template_id = Strings::ToUnsignedInt(sep->arg[2]);
		const uint32_t event_id = Strings::ToUnsignedInt(sep->arg[3]);
		const uint32_t npc_type_id = Strings::ToUnsignedInt(sep->arg[4]);
		const uint32_t spawn2_id = Strings::ToUnsignedInt(sep->arg[5]);
		...
		ShowMappedNpcManageMenu(c, *template_data, *event_data, *event_npc);
}
...
void HandleFix(Client* c, const Seperator* sep)
{
		...
		if (action == "zonein") {
			...
			const uint32_t seconds = sep->arg[3][0] ? ParseExpeditionDuration(sep->arg[3]) : ExpeditionDB::kSimpleSetupReplaySeconds;
...
void HandleDisable(Client* c, const Seperator* sep)
{
		...
		if (sub == "disable" && template_data->enabled && strcasecmp(sep->arg[3], "confirm") != 0) {
			SendCommandConfirmationPopup(c, "Unpublish Expedition", {
				fmt::format("Unpublish live expedition [{}]?", template_data->name),
				"Players may currently be inside it.",
				"Request NPCs and runtime hooks will stop using this expedition."
			}, fmt::format("#expedition disable {} confirm", template_data->id), "Unpublish");
			return;
		}
		const std::string template_name = template_data->name;
...
void HandleDelete(Client* c, const Seperator* sep)
{
		...
		if (strcasecmp(sep->arg[3], "confirm") != 0 && strcasecmp(sep->arg[4], "confirm") != 0) {
			SendCommandConfirmationPopup(c, "Delete Expedition", {
				fmt::format("Permanently delete draft expedition [{}]?", delete_template->name),
				"This deletes the template and linked setup rows.",
				"This cannot be undone from this menu."
			}, fmt::format("#expedition delete {} confirm", delete_template->id), "Delete");
			return;
		}
...
void HandleEventGroup(Client* c, const Seperator* sep, const ExpeditionDB::Template& template_data)
{
		...
		if (IsStrictUnsigned(sep->arg[3])) {
			...
			if (!ExpeditionDB::IsGroupedBossCompletionMode(*event_data)) {
				c->Message(Chat::Red, "That event is not a grouped boss event.");
				return;
			}
			ExpeditionDB::SetSelectedEvent(c->CharacterID(), event_id);
		...
		const bool add_to_explicit_event = sub_action == "add" && IsStrictUnsigned(sep->arg[4]);
		const uint32_t explicit_event_id = add_to_explicit_event ? Strings::ToUnsignedInt(sep->arg[4]) : 0;
		const auto* event_data = add_to_explicit_event ? FindTemplateEventByID(template_data, explicit_event_id) : RequireSelectedEvent(c, template_data);
		if (!event_data) {
			if (add_to_explicit_event) {
				c->Message(Chat::Red, "Boss group event not found.");
			}
			return;
		}

		if (!ExpeditionDB::IsGroupedBossCompletionMode(*event_data)) {
			c->Message(Chat::Red, add_to_explicit_event ? "That event is not a grouped boss event." : "Select or create a grouped boss event first.");
			ShowGroupedBossEventCard(c, nullptr);
			return;
		}
		...
		ExpeditionDB::SetSelectedEvent(c->CharacterID(), selected_event_id);
		...
		if (sub_action == "add") {
			NPC* npc = RequireTargetNpc(c, add_to_explicit_event ? "Target a boss NPC, then click Add Target to Group again." : "Target a boss NPC, then click Add Target Boss again.");
			if (!npc) {
				ShowGroupedBossEventCard(c, event_data);
				return;
			}
		...
		if (sub_action == "remove") {
			...
			if (strcasecmp(sep->arg[4], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Remove Group Boss", {
					fmt::format("Remove boss [{}] from grouped event [{}]?", NpcLabel(*npc), selected_event_name),
					"This changes the boss group completion requirements."
				}, "#expedition event group remove confirm", "Remove");
				return;
			}
		...
		if (sub_action == "removechest") {
			...
			if (strcasecmp(sep->arg[4], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Remove Group Chest", {
					fmt::format("Remove chest trigger [{}] from grouped event [{}]?", NpcLabel(*npc), selected_event_name),
					"This changes the grouped boss completion trigger setup."
				}, "#expedition event group removechest confirm", "Remove");
				return;
			}
...
void HandleEvent(Client* c, const Seperator* sep)
{
		...
		if (action == "list") {
			...
			for (const auto& event_data : template_data->events) {
				...
				if (ExpeditionDB::IsGroupedBossCompletionMode(event_data)) {
					event_actions.emplace_back(fmt::format("#expedition event group add {}", event_data.id), "Add Target to Group");
					event_actions.emplace_back(fmt::format("#expedition event group {}", event_data.id), "Manage Boss Group");
				}
				event_actions.emplace_back(fmt::format("#expedition event remove {}", event_data.id), "Delete Event");
				SendActionRow(c, event_actions);
				SendPrefillActionLink(c, fmt::format("#expedition event rename {} \"{}\"", event_data.id, event_data.event_name), "Rename", "edit event name");
			}
			RefreshBuilderView(c);
			return;
		}
		...
		if (action == "remove" || action == "delete") {
			...
			const std::string delete_target = has_target && target_confirm ? TailArg(sep->arg[3]) : CommandTail(sep, 3);
			const auto* delete_event = has_target ? ExpeditionDB::FindEvent(*template_data, delete_target) : ExplicitSelectedEvent(c, *template_data);
			...
			if (!bare_confirm && !target_confirm) {
				SendCommandConfirmationPopup(c, "Delete Event", {
					fmt::format("Delete event [{}] id [{}]?", delete_event->event_name, delete_event->id),
					fmt::format("NPC mappings: {} | runtime actions: {}", delete_event->npcs.size(), delete_event->actions.size()),
					"This removes the event, mapped NPCs, and runtime actions from the DB template.",
					"This cannot be undone from this menu."
				}, fmt::format("#expedition event remove {} confirm", delete_event->id), "Delete");
				return;
			}
		...
		if (action == "lockout") {
			if (sep->arg[3][0] == '\0') {
				...
				const bool grouped = ev && ExpeditionDB::IsGroupedBossCompletionMode(*ev);
				ShowTimeMatrix(c, fmt::format("Boss Lockout: {}", selected_event_name),
					"#expedition event lockout", ev ? ev->lockout_seconds : 0, true,
					grouped ? fmt::format("#expedition event group {}", selected_event_id) : "#expedition event list",
					grouped ? "Back to Boss Group" : "Back to Events");
				return;
			}
			const bool clear = IsClearDurationArg(sep->arg[3]);
			const uint32_t seconds = clear ? 0 : ParseExpeditionDuration(sep->arg[3]);
			...
			if (action == "npc") {
				if (Strings::EqualFold(sep->arg[3], "remove")) {
					...
					if (strcasecmp(sep->arg[4], "confirm") != 0) {
						...
						SendCommandConfirmationPopup(c, removes_event ? "Remove Boss And Event" : "Remove Event NPC", {
							fmt::format("Remove targeted NPC [{}] from event [{}]?", NpcLabel(*npc), selected_event_name),
							removes_event ? "This also deletes the event because this NPC is its only boss mapping." : "This removes the NPC mapping from the selected event."
						}, "#expedition event npc remove confirm", "Remove");
						return;
					}
					const bool removes_event = ExpeditionDB::EventNpcRemovalDeletesEvent(*event_data, *mapped_npc);
...
void HandleAction(Client* c, const Seperator* sep)
{
		...
		if (action == "clear") {
			if (strcasecmp(sep->arg[3], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Clear Runtime Actions", {
					fmt::format("Remove every runtime action from selected event [{}]?", selected_event_name),
					"This changes the DB event action list.",
					"This cannot be undone from this menu."
				}, "#expedition action clear confirm", "Clear");
				return;
			}
...
void HandlePreset(Client* c, const Seperator* sep)
{
		...
		if (preset == "solo") {
			...
			ExpeditionDB::SetDzTemplateDuration(content_db, dz_template_id, ExpeditionDB::kSimpleSetupDurationSeconds);
			ExpeditionDB::SetTemplateReplay(content_db, template_id, ExpeditionDB::kSimpleSetupReplaySeconds);
...
void HandleTest(Client* c, const Seperator* sep)
{
		...
		if (action == "create") {
			if (strcasecmp(sep->arg[3], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Create Live Test Expedition", {
					fmt::format("Create a live test expedition from [{}] for your group/raid/self?", template_data->name),
					"This creates live expedition state."
				}, "#expedition test create confirm", "Create");
				return;
			}
			...
			if (strcasecmp(sep->arg[3], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Move Into Expedition", {
					"Move into your current expedition now?",
					"This changes your current zone/instance."
				}, "#expedition test move confirm", "Move");
				return;
			}
			...
			if (strcasecmp(sep->arg[3], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Apply Event Lockout", {
					"Apply the selected event lockout to your current expedition?",
					"This changes live expedition lockout state."
				}, "#expedition test lockout confirm", "Apply");
				return;
			}
			...
			if (strcasecmp(sep->arg[3], "confirm") != 0) {
				SendCommandConfirmationPopup(c, "Apply Loot Protection", {
					"Re-apply DB loot-event protection to your current expedition?",
					"This changes live expedition loot protection state."
				}, "#expedition test loot confirm", "Apply");
				return;
			}
			if (DynamicZone* dz = c->GetExpedition()) {
